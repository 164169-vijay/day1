...
public class DateDifferenceProvider {
	...
	public int getDateDifference(MyDate startDate, MyDate endDate) {
		...
		} else if ((sameYear(startDate, endDate)) && diffmonth(startDate, endDate) && containFeb(startDate.getMm())) {
		...
		else if (diffYear(startDate, endDate) && (startDate.getMm() <= 2) && (endDate.getMm() > 2)) {
			return remainingDaysOfMonth(startDate) + remainingDaysInYear(startDate) + leadingYear(endDate)
					+ daysInInterveningYear(startDate, endDate) + isLeapYear(startDate.getYyyy())
					+ isLeapYear(endDate.getYyyy());
		}

		else if (diffYear(startDate, endDate) && (startDate.getMm() <= 2)) {
			return remainingDaysOfMonth(startDate) + remainingDaysInYear(startDate) + leadingYear(endDate)
					+ daysInInterveningYear(startDate, endDate) + isLeapYear(startDate.getYyyy());
		} else if (diffYear(startDate, endDate) && (endDate.getMm() > 2)) {
			return remainingDaysOfMonth(startDate) + remainingDaysInYear(startDate) + leadingYear(endDate)
					+ daysInInterveningYear(startDate, endDate) + isLeapYear(endDate.getYyyy());
		}
		return 0;
	...
	private boolean diffmonth(MyDate startDate, MyDate endDate) {
		return endDate.getMm() != startDate.getMm();
	}
	...
	private boolean diffYear(MyDate startDate, MyDate endDate) {
		return endDate.getYyyy() != startDate.getYyyy();
	}

	private boolean containFeb(int month) { // same year
		return (month <= 2);
	}

	private boolean notcontainFeb(int month) { // same year
		return (month >= 3);
	}
	// //For same Century
	// private boolean sameCentury(MyDate startDate, MyDate endDate) {
	// return endDate.getYyyy() == startDate.getYyyy();
	// }
	//
	//
	...
	private int daysInInterveningYear(MyDate startDate, MyDate endDate) {
		...
		for (int year = startDate.getYyyy() + 1; year < endDate.getYyyy(); year++) {
			...
			remainingdays = remainingdays + 365 + isLeapYear(year);
		}
